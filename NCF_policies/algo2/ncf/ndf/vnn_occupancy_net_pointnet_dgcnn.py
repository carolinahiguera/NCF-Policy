...
from algo.ncf.ndf.layers_equi import *
...
class DecoderCBatchNorm(nn.Module):
    ...
    def __init__(
        self, dim=3, z_dim=128, c_dim=128, hidden_size=256, leaky=False, legacy=False
    ):
        super().__init__()
        self.z_dim = z_dim
        if not z_dim == 0:
...
class DecoderCBatchNormNoResnet(nn.Module):
    ...
    def __init__(self, dim=3, z_dim=128, c_dim=128, hidden_size=256, leaky=False):
        super().__init__()
        self.z_dim = z_dim
        if not z_dim == 0:
            self.fc_z = nn.Linear(z_dim, hidden_size)
...
class AffineLayer(nn.Module):
    ...
    def reset_parameters(self):
        ...
        with torch.no_grad():
            ...
            self.fc_b.bias.copy_(torch.tensor([0.0, 0.0, 2.0]))
