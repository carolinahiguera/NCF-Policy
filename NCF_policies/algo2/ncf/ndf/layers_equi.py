...
# print("=============================")
# print(f'layers equi device {device}')
# print("=============================")
...
def get_graph_mean(x, k=20, idx=None):
    ...
    feature = x.view(batch_size * num_points, -1)[idx, :]
    feature = feature.view(batch_size, num_points, k, num_dims, 3).mean(
        2, keepdim=False
    )
...
def get_shell_mean_cross(x, k=10, nk=4, idx_all=None):
    ...
    for i in range(nk):
        ...
        feature[i] = (
            feature[i]
            .view(batch_size, num_points, k, num_dims, 3)
            .mean(2, keepdim=False)
        )
